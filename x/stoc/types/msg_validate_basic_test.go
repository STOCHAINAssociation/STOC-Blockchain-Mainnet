...
// TestNativeSymbol_CaseInsensitive_Round21 pins the round-21 (2026-07-13) fix for
// the round-20 regression: IsNativeDenom was made case-SENSITIVE and (wrongly)
// reused to validate user-supplied token Symbols, letting uppercase native tickers
// ("STOC"/"USTOC") clone the native gas asset at the wallet/explorer layer. The
// three Symbol validators (MsgCreateToken.ValidateBasic, Token.Validate,
// Token.ValidateState) must reject native symbols CASE-INSENSITIVELY, while
// IsNativeDenom stays case-SENSITIVE for the bank-denom SendRestriction gates
// (opposite polarity — bank denoms are unique SYMBOL_<counter>).
//
// Note: astoc (EVM denom) is not asserted here because SafeGetEvmDenom is
// uninitialised in a pure types unit test; the cosmos ("ustoc") + display
// ("stoc") variants fully exercise the case-insensitivity fix. astoc case
// variants are covered end-to-end by the devnet round-21 replay.
func TestNativeSymbol_CaseInsensitive_Round21(t *testing.T) {
	oldBondDenom := sdk.DefaultBondDenom
	sdk.DefaultBondDenom = "ustoc"
	defer func() { sdk.DefaultBondDenom = oldBondDenom }()
	...
	// --- Symbol polarity: case-INSENSITIVE reserved-symbol match ---
	for _, s := range []string{"stoc", "STOC", "Stoc", "sTOc", "ustoc", "USTOC", "Ustoc", "uStOc"} {
		require.True(t, types.IsNativeSymbol(s), "IsNativeSymbol(%q) must be true (case-insensitive native ticker)", s)
	}
	for _, s := range []string{"MYT", "STOCK", "USTOCK", "STOCKS", "FOO", "STOCX"} {
		require.False(t, types.IsNativeSymbol(s), "IsNativeSymbol(%q) must be false (not a native ticker)", s)
	}

	// --- Denom polarity: IsNativeDenom stays case-SENSITIVE (round-20 intent preserved) ---
	require.True(t, types.IsNativeDenom("ustoc"), "canonical cosmos denom still native")
	require.True(t, types.IsNativeDenom("stoc"), "canonical display denom still native")
	require.False(t, types.IsNativeDenom("USTOC"), "uppercase bank denom is a distinct (non-native) coin — gates must NOT skip restriction")
	require.False(t, types.IsNativeDenom("STOC"), "uppercase display is a distinct bank denom, not native at the denom gate")

	// --- Validator polarity: all three creation validators reject native symbols case-insensitively ---
	for _, sym := range []string{"STOC", "USTOC", "Stoc", "Ustoc", "stoc", "ustoc"} {
		msg := types.MsgCreateToken{
			Creator:       creator,
			Name:          "Impersonator",
			Symbol:        sym,
			...
			TotalSupply:   math.NewInt(1000),
			Unlimited:     false,
			Distributions: []types.WalletDistribution{{Address: creator, Percent: 100}},
		}
		errVB := msg.ValidateBasic()
		require.Error(t, errVB, "ValidateBasic must reject native symbol %q", sym)
		require.Contains(t, errVB.Error(), "symbol cannot be a native denom", "symbol %q", sym)

		errV := types.Validate(types.Token{Name: "Impersonator", Symbol: sym})
		require.Error(t, errV, "Validate must reject native symbol %q", sym)
		require.Contains(t, errV.Error(), "conflicts with native chain denom", "symbol %q", sym)

		errS := types.ValidateState(types.Token{Name: "Impersonator", Symbol: sym})
		require.Error(t, errS, "ValidateState must reject native symbol %q", sym)
		require.Contains(t, errS.Error(), "conflicts with native chain denom", "symbol %q", sym)
	}

	// --- Regression guard: a legit non-native symbol still passes the native check ---
	require.False(t, types.IsNativeSymbol("MYT"), "legit symbol must not be over-blocked")
}
